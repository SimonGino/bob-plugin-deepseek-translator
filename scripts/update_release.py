...
def update_appcast(version, desc):
    release_file = Path(f'release/deepseek-translator-{version}.bobplugin')
    ...
    version_info = {
        ...
        'desc': desc,
        'sha256': file_hash,
        'url': f'https://github.com/simongino/bob-plugin-deepseek-translator/releases/download/v{version}/{release_file.name}',
        'minBobVersion': '0.5.0'
    }
    ...
    if appcast_file.is_file():
        ...
        appcast = dict(identifier='simongino.deepseek.translator', versions=[])
